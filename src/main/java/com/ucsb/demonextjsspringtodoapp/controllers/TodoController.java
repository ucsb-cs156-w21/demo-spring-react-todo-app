...
public class TodoController {
  ...
  public ResponseEntity<String> updateTodo(@RequestHeader("Authorization") String authorization,
      @PathVariable("id") Long id, @RequestBody @Valid Todo todo) throws Exception {
    DecodedJWT jwt = JWT.decode(authorization.substring(7));
    Optional<Todo> existingTodo = todoRepository.findById(id);

    // Check that the user actually owns the todo to be updated.
    if (!existingTodo.isPresent() || !existingTodo.get().getUserId().equals(jwt.getSubject())) {
      return ResponseEntity.notFound().build();
    }
    if (todo.getId() != existingTodo.get().getId()) {
